...
std::string memory_status()
{
    ...
    for (int i = 0; i < 6; ++i)
    {
        const auto &p = memory_paritions[i];
        total_usable += p.size;

        if (p.occupied == -1)
        {
            total_free += p.size;
            ss << "Partition " << p.partition_number
               << " (" << p.size << " MB): FREE\n";
        }
        else
        {
            total_used += p.size;
            ss << "Partition " << p.partition_number
               << " (" << p.size << " MB): used by PID "
               << p.occupied << "\n";
        }
    }

    ss << "Total used memory: " << total_used << " MB\n";
    ss << "Total free memory: " << total_free << " MB\n";
    ss << "Total usable memory: " << total_usable << " MB\n\n";
